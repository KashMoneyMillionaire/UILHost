...
namespace UILHost.Common
{
    ...
    {
        readonly Section _rootSection = null;
        ...
        public string this[string token]
        {
            get 
            {
                ...
                var path = token.Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries);

                var currentSection = this._rootSection;
        ...
        public TokenValuesCollection(string tokenValuesDataPath)
        {
            ...
            try
            {
                ...
                using (Stream dataStream = File.OpenRead(tokenValuesDataPath))
                    this._rootSection = (Section)serializer.Deserialize(dataStream);
            }
            catch (Exception ex)
            {
